...
from online_video_llm.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class LlavaMetaForCausalLM(ABC):
    ...
    def extract_images(self, images, vision_encoder=None):
        image_features_list = []
        block_size = 16
        for i in range(0, images.shape[0], block_size):
            image_features = self.get_model().get_vision_tower()(images[i: i+block_size]) if vision_encoder is None else vision_encoder(images[i: i+block_size])
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, qs_ids, qs_mask, past_key_values, labels, images, projector, images_large=None, vision_encoder=None, ans_token=None, todo_token=None
    ):
        ...
        # Accelerate may attach a CPU execution hook to the outer compressor
        # while dispatching the nested token embedding to a GPU. Align every
        # text-side tensor with the embedding weight at the last responsible
        # boundary instead of relying on the caller's generic CUDA device.
        embed_tokens = self.get_model().embed_tokens
        text_device = embed_tokens.weight.device

        def to_text_device(value):
            if torch.is_tensor(value) and value.device != text_device:
                return value.to(device=text_device, non_blocking=True)
            return value

        input_ids = to_text_device(input_ids)
        position_ids = to_text_device(position_ids)
        attention_mask = to_text_device(attention_mask)
        qs_ids = to_text_device(qs_ids)
        qs_mask = to_text_device(qs_mask)
        labels = to_text_device(labels)
        ans_token = to_text_device(ans_token)
        todo_token = to_text_device(todo_token)
        ...
        if qs_ids is not None:
            qs_embeds = embed_tokens(qs_ids)
        else:
            qs_embeds = None
        
        if ans_token is not None:
            ans_embed = embed_tokens(ans_token)
            todo_embed = embed_tokens(todo_token)

        assert len(images) == len(input_ids)

        # list of mixed images and equal length videos
        # if type(images) is list:
        #     concat_images = []
        #     concat_videos = []
        #     modality_indicators = []
        ...
        if type(images) is list:
            ...
            for image in images:
                if image.ndim == 3: # single image
                    concat_images.append(image.unsqueeze(0)) # 1 c h w
                    modality_indicators.append(1)
                elif image.ndim == 4: # multiple frames
                    concat_images.append(image) # t c h w
                    modality_indicators.append(2)
            concat_images = torch.cat(concat_images, dim=0) # n c h w
            mix_image_video = self.extract_images(concat_images) # n k c
            mix_image_video = self.mix_spatial_tokens(mix_image_video) # n k c
            clip_features = []
            image_index = 0
            for image in images:
                if image.ndim == 3:
                    clip_features.append(mix_image_video[image_index]) # k c
                    image_index += 1
                elif image.ndim == 4:
                    if not base_mode:
                        clip_features.append(mix_image_video[image_index: image_index+image.shape[0]].view(-1, mix_image_video.shape[-1])) # tk c
                    else:
                        clip_features.append(mix_image_video[image_index: image_index+image.shape[0]]) # t k c
                    image_index += image.shape[0]
            clip_features = [x.to(self.device) for x in clip_features]
            image_features = self.project_features(clip_features)
            image_features = [x.to(self.device) for x in image_features]
        elif images.ndim == 5:
        ...
        if images_large is not None and False:
            if type(images_large) is list:
                concat_images = []
                concat_videos = []
                for image in images_large:
                    if image.ndim == 3: # single image
                        concat_images.append(image.unsqueeze(0))
                    elif image.ndim == 4: # multiple frames
                        concat_videos.append(image)
                ...
                concat_videos = torch.stack(concat_videos, dim=0) # n t c h w
                mix_image_video = torch.cat([concat_images, concat_videos.view(-1, *concat_videos.shape[2:])], dim=0) # m c h w
                mix_image_video = self.extract_images(mix_image_video, vision_encoder) # m k c
                if not base_mode:
                    mix_image_video = self.mix_spatial_tokens(mix_image_video)
                    concat_images = mix_image_video[:concat_images.shape[0]].contiguous() # n k c
                    concat_videos = mix_image_video[concat_images.shape[0]:].contiguous().view(
                ...
                for image in images:
                    if image.ndim == 3:
                        ...
                        video_index += 1
                clip_features = [x.to(self.device) for x in clip_features]
            elif images_large.ndim == 5:
                concat_images = images_large.view(-1, *images_large.shape[2:]) # nt c h w
                clip_features = self.extract_images(concat_images, vision_encoder)
                if not base_mode:
                    clip_features = self.mix_spatial_tokens(clip_features) # nt k c
                    clip_features = clip_features.view(images_large.shape[0], images_large.shape[1]*clip_features.shape[1], clip_features.shape[2]) # n tk c
                else:
                    clip_features = self.mix_spatial_tokens(clip_features) # nt k c
                    clip_features = clip_features.view(images_large.shape[0], images_large.shape[1], clip_features.shape[1], clip_features.shape[2]) # n t k c
                clip_features = [x.to(self.device) for x in clip_features]
            else:
                clip_features = self.extract_images(images_large, vision_encoder)
                clip_features = self.mix_spatial_tokens(clip_features)
                clip_features = [x.to(self.device) for x in clip_features]
        ...
        if base_mode:
            ...
            return None, position_ids, attention_mask, past_key_values, new_input_embeds, clip_features, qs_embeds, qs_mask, (new_labels, new_indicators), ans_embed, todo_embed
