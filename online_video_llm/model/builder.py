...
import torch
from online_video_llm.model import *
from online_video_llm.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs):
    ...
    # elif 'long' in model_name.lower() and 'ground' not in model_name.lower():
    #     tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, trust_remote_code=True)
    #     if 'intern' in model_name.lower():
    #         model = LongInternLM2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
    #     elif 'qwen' in model_name.lower():
    #         model = LongQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
    # elif 'ground' in model_name.lower():
    #     tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
    #     if 'qwen' in model_path.lower():
    #         model = GroundQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
    #     else:
    #         model = GroundPhiForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
    # else:
    #     # Load language model
    ...
    # if 'long' in model_name.lower() and 'ground' not in model_name.lower():
    mm_use_im_start_end = getattr(model.config, "mm_use_im_start_end", False)
    mm_use_im_patch_token = getattr(model.config, "mm_use_im_patch_token", True)
    if mm_use_im_patch_token:
        tokenizer.add_tokens([DEFAULT_IMAGE_PATCH_TOKEN], special_tokens=True)
